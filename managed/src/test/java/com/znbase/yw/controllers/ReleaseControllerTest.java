...
import com.ZNbase.yw.common.ReleaseManager;
...
import static com.ZNbase.yw.common.AssertHelper.assertInternalServerError;
...
import static com.ZNbase.yw.common.ReleaseManager.ReleaseState.DISABLED;
...
public class ReleaseControllerTest extends FakeDBApplication {
  ...
  private void mockReleaseData(boolean multiple) {
    ...
    if (multiple) {
      ReleaseManager.ReleaseMetadata activeRelease =
          ReleaseManager.ReleaseMetadata.fromLegacy("0.0.3", "ZNbase-0.0.3.tar.gz");
      ReleaseManager.ReleaseMetadata disabledRelease =
          ReleaseManager.ReleaseMetadata.fromLegacy("0.0.2", "ZNbase-0.0.2.tar.gz");
      disabledRelease.state = DISABLED;
      ReleaseManager.ReleaseMetadata deletedRelease =
          ReleaseManager.ReleaseMetadata.fromLegacy("0.0.1", "ZNbase-0.0.1.tar.gz");
  ...
  public void testGetReleasesWithMetadata() {
    ...
    Map expectedMap = ImmutableMap.of(
        "0.0.1",
        ImmutableMap.of("filePath", "ZNbase-0.0.1.tar.gz",
            "state", "ACTIVE")
    );
  ...
  public void testGetReleasesStateFilteringWithMetadata() {
    ...
    Map expectedMap = ImmutableMap.of(
        "0.0.3",
        ImmutableMap.of("filePath", "ZNbase-0.0.3.tar.gz",
            "state", "ACTIVE"),
        "0.0.2",
        ImmutableMap.of("filePath", "ZNbase-0.0.2.tar.gz",
            "state", "DISABLED")
    );
    Result result = getReleases(customer.uuid, true);
    JsonNode json = Json.parse(contentAsString(result));
