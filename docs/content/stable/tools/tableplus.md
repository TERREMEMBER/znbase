---
title: Use TablePlus with ZNbaseDB YSQL and YCQL
headerTitle: TablePlus
linkTitle: TablePlus
description: Learn how to connect TablePlus to ZNbaseDB and query using YSQL, YCQL, and YEDIS.
menu:
  stable:
...
In this tutorial, you will show how to connect TablePlus with all three ZNbaseDB APIs on a local cluster. TablePlus works without any issues with ZNbaseDB because the ZNbaseDB APIs are compatible at the wire protocol level with databases already supported by TablePlus.
...
Follow the instructions in the [Quick Start](../../../quick-start/install) to create a local ZNbaseDB cluster. We will use the macOS install instructions for the rest of this tutorial.
...
Now enter the connection details as shown in the screenshot below and then click `Test` to ensure that TablePlus is indeed able to establish connectivity with the ZNbaseDB API. The color of the fields will change to green if the test succeeds.
...
Now you have connected to all the ZNbaseDB APIs and can start exploring them by simply double-clicking on the connection name.
...
[tableplus integration: redis metadata commands should gracefully error for redis compatible yedis](https://github.com/ZNbase/ZNbase-db/issues/503)
