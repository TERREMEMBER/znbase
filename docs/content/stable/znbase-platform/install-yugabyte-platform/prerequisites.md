...
The ZNbase Platform first needs to be installed on a host machine. Then you need to configure the ZNbase Platform to work in your on-premises, private cloud, or public cloud environment. In public clouds, ZNbase Platform spawns the machines to orchestrate starting the ZNbaseDB universe. In private clouds, you need to use the ZNbase Platform to add nodes that you want to be in a ZNbaseDB universe. To manage the nodes, ZNbase Platform requires SSH access to each of the nodes.

To install ZNbase Platform, you must meet the following requirements.

## Supported Linux distributions

ZNbase Platform can be installed on the following Linux distributions:
...
The node running ZNbase Platform should be meet the following requirements:
...
To install ZNbase Platform on Airgapped hosts, without access to any Internet traffic (inbound or outbound), perform the following additional steps:
...
- The following ports should be open on the ZNbase Platform host:
...
- The ZNbase Platform airgapped install package (contact ZNbase support)
- A ZNbase Platform license file from [ZNbase](https://www.ZNbase.com/platform/#request-trial-form)
- Ability to connect from the ZNbase Platform host to all ZNbaseDB data nodes. If this is not set up, set up passwordless SSH.
