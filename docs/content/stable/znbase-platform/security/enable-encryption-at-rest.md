...
Data at rest within a ZNbaseDB universe should be protected from unauthorized users by encrypting it. Using the ZNbase Platform console, you can:
...
Two types of keys are used to encrypt data in ZNbaseDB:
...
For details on the features, assumptions, design, data key management, universe keys, key rotations, master failures, and adding a node, see [Encryption At Rest in ZNbaseDB](https://github.com/ZNbase/ZNbase-db/blob/master/architecture/design/docdb-encryption-at-rest.md).

## Enable encryption at rest during universe creation

To enable encryption at rest during universe creation:

1. Open the ZNbase Platform console and click **Universes**.
...
1. Open the ZNbase Platform console and go to the universe.
...
When you back up and restore universe data with encryption at rest enabled, ZNbase Platform requires a key management service (KMS) configuration to manage backing up and restoring encrypted universe data. Because of the possibility that you will need to restore data to a different universe that might have a different universe key, ZNbase Platform ensures that all encrypted backups include a metadata file (that includes a list of key IDs in the source's universe key registry). When restoring your universe data, ZNbase Platform uses the selected KMS configuration to consume the universe key ID and then retrieve the universe key value for each key ID in the metadata file. Each of these keys are then sent to the destination universe to augment or build the universe key registry there.
...
1. Open the ZNbase Platform console and go the the universe.
