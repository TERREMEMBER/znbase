...
ZNbase Platform allows you to protect data in transit by using the following:
...
ZNbase Platform lets you create a new self-signed certificate, use an existing self-signed certificate, or upload a third-party certificate from external providers, such as Venafi or DigiCert (which is only available for an on-premise cloud provider).
...
## Self-Signed Certificates Generated by ZNbase Platform

ZNbase Platform can create self-signed certificates for each universe. These certificates may be shared between universes within a single instance of ZNbase Platform. The certificate name has the following format: 
...
You can enable TLS using certificates generated by ZNbase Platform, as follows:
...
- That the file names and file paths of different certificates and keys are identical across all the database nodes. For example, if you name your CA root certificate as `ca.crt` on one node, then name it `ca.crt` on all the nodes. Similarly, if you copy `ca.crt` to `/opt/ZNbase/keys` on one node, then copy `ca.crt` to the same path on other nodes.
- That the ZNbase system user has read permissions to all the certificates and keys. 

**Step 3**: Create a CA-signed certificate in ZNbase Platform, as follows:
...
The following procedure describes how to rotate the existing custom certificates and replace them with new database node certificates issued by the same custom CA that issued the original certificates. Note that in the current release, ZNbase Platform does not support changing the root CA certificate.
...
**Step 3**: Follow Step 3 of [How to Use Custom CA-Signed Certificates to Enable TLS](#how-to-use-custom-ca-signed-certificates-to-enable-tls) to create a new CA-signed certificate in ZNbase Platform.
...
- Click **Actions** and select **Download YSQL Cert**, as shown in the following illustration. This triggers the download of the `ZNbasedb.crt` and `ZNbasedb.key` files.

![download-ysql-cert](/images/yp/encryption-in-transit/download-ysql-cert.png)

- Optionally, when connecting to universes that are configured with custom CA-signed certificates, obtain the root CA and client YSQL certificate from your administrator. These certificates are not available on ZNbase Platform for downloading. 

- For testing with a `ysqlsh` client, paste the `ZNbasedb.crt` and `ZNbasedb.key` files into the `<home-dir>/.ZNbasedb` directory and change the permissions to `0600`, as follows:
...
To use TLS from a different client, consult the client-specific documentation. For example, if you are using a Postgres JDBC driver to connect to ZNbaseDB, see [Configuring the Client](https://jdbc.postgresql.org/documentation/head/ssl-client.html) for more details.
...
To use TLS from a different client, consult the client-specific documentation. For example, if you are using a Cassandra driver to connect to ZNbaseDB, see [SSL](https://docs.datastax.com/en/developer/python-driver/3.19/security/#ssl) .
