...
The **Provider**, **Regions** and **Instance Type** fields were initialized based on the [cloud providers configured](../../configure-ZNbase-platform/set-up-cloud-provider/). As soon as **Provider**, **Regions** and **Nodes** are entered, an intelligent Node Placement Policy kicks in to specify how the nodes should be placed across all the Availability Zones so that maximum availability is guaranteed.

We are going to enter the following values to create a multi-zone universe on [GCP](../../configure-ZNbase-platform/set-up-cloud-provider/gcp) provider:
...
Export an environment variable telling the IP addresses for nodes in the cluster. Browse to the **Universe Overview** tab in the ZNbase Platform console and click **YCQL Endpoints**. A new tab opens with a list of IP addresses.

![YCQL end points](/images/ee/multi-zone-universe-ycql-endpoints.png)

Export this into a shell variable on the database node `yb-dev-helloworld1-n1` you had connected to. Remember to replace the IP addresses below with those shown by the ZNbase Platform console.
...
Browse to the **Metrics** tab of the universe in the ZNbase Platform console. You should be able to see the metrics show up. The metrics tab displays a variety of metrics, a few are shown in the screenshot below. Note that these numbers (server side metrics) tally with what the load tester reports (client side metrics).
