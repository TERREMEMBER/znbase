...
```plpgsql
ZNbase=# CREATE AGGREGATE sumdouble (float8) (
              STYPE = float8,
              SFUNC = float8pl,
              MSTYPE = float8,
              MSFUNC = float8pl,
...
ZNbase=# CREATE TABLE normal_table(
             f float8,
             i int
           );
ZNbase=# INSERT INTO normal_table(f, i) VALUES
             (0.1, 9),
             (0.9, 1);
ZNbase=# SELECT sumdouble(f), sumdouble(i) FROM normal_table;
```

Order by syntax example.

```plpgsql
ZNbase=# CREATE AGGREGATE my_percentile_disc(float8 ORDER BY anyelement) (
             STYPE = internal,
...
ZNbase=# SELECT my_percentile_disc(0.1), my_percentile_disc(0.9)
...
```plpgsql
ZNbase=# CREATE AGGREGATE oldcnt(
             SFUNC = int8inc,
             BASETYPE = 'ANY',
             STYPE = int8,
             INITCOND = '0'
           );
ZNbase=# SELECT oldcnt(*) FROM pg_aggregate;
```
...
ZNbase=# SELECT newcnt(*) FROM pg_aggregate;
```
