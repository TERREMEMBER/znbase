...
The [`recursive-cte-code-examples.zip`](https://raw.githubusercontent.com/ZNbase/ZNbase-db/master/sample/recursive-cte-code-examples/recursive-cte-code-examples.zip) file contains the `.sql` scripts that illustrate the use off the [recursive CTE](./recursive-cte/):
