...
`cqlsh` is a command line interface (CLI), or shell, for interacting with ZNbaseDB through [YCQL](../../api/ycql/).

## Download

cqlsh is installed as part of ZNbaseDB and is located in the `bin` directory of ZNbase home. It is also available for download and install from ZNbaseDB's [GitHub repo](https://github.com/ZNbase/cqlsh/releases).
