...
To run a distributed SQL cluster locally, run the following `yb-docker-ctl` command to create a 3-node ZNbaseDB cluster with a replication factor (RF) of 3.
...
Run the command below to see that we now have 1 `yb-master` (yb-master-n1) and 1 `yb-tserver` (yb-tserver-n1) containers running on this localhost. Roles played by these containers in a ZNbaseDB cluster are explained in detail [here](../../architecture/concepts/universe/).
...
The yb-master-n1 home page shows that we have a cluster (aka a Universe) with `Replication Factor` of 1 and `Num Nodes (TServers)` as 1. The `Num User Tables` is 0 since there are no user tables created yet. ZNbaseDB version number is also shown for your reference.
