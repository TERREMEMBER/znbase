...
Software Installed | During universe (or cluster) creation, this indicates ZNbase software is being deployed on this node.
...
The second scenario is for more of a 'quick' planned change that can be performed on a node. For example, the DevOps wants to mount a new disk on the node or just install and run a new security daemon. In that case, the instance is still in use and stopping any running ZNbase process might be needed. Then the user can pick the `Stop Processes` option and then perform the system task, and then pick the `Start Processes` for that node.
