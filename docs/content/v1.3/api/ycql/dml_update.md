...
The `UPDATE` statement updates one or more column values for a row in table. Currently, ZNbase can only update one row at a time, updating multiple rows is not yet supported.
