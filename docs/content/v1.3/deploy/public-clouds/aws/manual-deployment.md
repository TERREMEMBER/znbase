...
Now that the 6 nodes have been prepared, the yb-master process will be run on 3 of these nodes (because RF=3) and yb-tserver will be run on all 6 nodes. To learn more about ZNbaseDB’s process architecture, see [here](../../../architecture/concepts/universe/).
...
# suitable limits and data directories for ZNbase to use then
...
On each of those nodes, first locate the SSD device(s) that’ll be used as the data directories for ZNbaseDB to store data on (such as RAFT/txn logs, SSTable files, logs, etc.)
...
Download the ZNbaseDB package, untar and run post install to patch relative paths on all nodes.
...
Create the ZNbaseDB `system_redis.redis` (which is the default Redis database 0) table using `yb-admin` or via `redis-cli`.
