...
Local storage gives great performance but the data is not replicated and can be lost if the node fails. This option is ideal for databases like ZNbaseDB that manage their own replication and can guarantee HA.
...
Thus, it is generally preferable to use local storage where possible for higher performance and lower costs. The following section explains how to deploy ZNbaseDB on Kubernetes using local SSDs.
