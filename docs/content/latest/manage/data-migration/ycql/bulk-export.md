...
This page documents bulk export for ZNbaseDB’s [Cassandra-compatible YCQL API](../../../../api/ycql). To export data from a ZNbaseDB (or an Apache Cassandra) table, you can use the [`cassandra-unloader`](https://github.com/ZNbase/cassandra-loader#cassandra-unloader) tool.

We will first create a source ZNbaseDB table and populate it with data. Then you will export the data out using the cassandra-unloader tool. We will use a generic gaming user profile use case as a running example to illustrate the export process.
