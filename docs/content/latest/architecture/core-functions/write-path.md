...
description: Learn how ZNbaseDB manages single-row write operations.
menu:
  latest:
...
ZNbaseDB has a [smart client](../../../develop/client-drivers/java/) that can cache the location of the
...
Note that the above scenario has been greatly simplified by assuming that the user application sends the write query to a random ZNbaseDB server, which then routes the request appropriately. In practice, the use of the ZNbaseDB smart client is recommended for removing the extra network hop.
