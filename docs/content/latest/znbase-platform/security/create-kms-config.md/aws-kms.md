...
description: Use ZNbase Platform to create a KMS configuration for Amazon Web Services (AWS) KMS.
aliases:
  - /latest/ZNbase-platform/security/create-kms-config
menu:
  latest:
    parent: security
...
Encryption at rest uses universe keys to encrypt and decrypt universe data keys. You can use the ZNbase Platform console to create key management service (KMS) configurations for generating the required universe keys for one or more ZNbaseDB universes. Encryption at rest in ZNbase Platform supports the use of [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/), from Amazon Web Services (AWS).
...
1. Open the ZNbase Platform console and click **Configs**.
...
4. Enter the following configuration details:
    ...
    - **Use IAM Profile** — Select to use an IAM profile attached to an EC2 instance running ZNbaseDB. For more information, see [Using instance profiles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use_switch-role-ec2_instance-profiles.html).
