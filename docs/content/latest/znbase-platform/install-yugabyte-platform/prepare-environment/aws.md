...
description: Prepare the Amazon Web Services (AWS) environment for the ZNbase Platform.
aliases:
  - /latest/ZNbase-platform/install-ZNbase-platform/prepare-environment/
menu:
  latest:
...
<ul class="nav nav-tabs-alt nav-tabs-yb">
  ...
  <li>
    <a href="/latest/ZNbase-platform/install-ZNbase-platform/prepare-environment/aws" class="nav-link active">
  ...
  <li>
    <a href="/latest/ZNbase-platform/install-ZNbase-platform/prepare-environment/azure" class="nav-link">
...
In order to access ZNbase Platform from outside the AWS environment, you would need to enable access by assigning an appropriate security group to the ZNbase Platform machine. You will at minimum need to:
...
In order for ZNbase Platform to manage ZNbaseDB nodes, limited access to your AWS infrastructure is required. To grant the required access, you can provide a set of credentials when configuring the AWS provider, as described in [Configure the AWS cloud provider](../../../configure-ZNbase-platform/set-up-cloud-provider/aws). Alternatively, the EC2 instance where the ZNbase Platform will be running can be brought up with an IAM role with enough permissions to take all the actions required by ZNbase Platform. Here is a sample of such a role:
...
## 3. Provision an instance for ZNbase Platform

Create an instance to run the ZNbase Platform server. In order to do so, go to **EC2 > Instances** and click **Launch Instance**. Fill in the following values.
