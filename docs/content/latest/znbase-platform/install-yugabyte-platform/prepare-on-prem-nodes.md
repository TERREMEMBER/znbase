...
For on-premises deployments of ZNbaseDB universes, you need to import nodes that can be managed by the ZNbase Platform. This page outlines the steps required to prepare these ZNbaseDB nodes for on-premises deployments.



1. Ensure that the ZNbaseDB nodes conform to the requirements outlined in the [deployment checklist](/latest/deploy/checklist/). This checklist also gives an idea of [recommended instance types across public clouds](/latest/deploy/checklist/#running-on-public-clouds). 
...
5. These are the actions that will be performed with sudo access.
    *   Create the `ZNbase:ZNbase` user + group
    *   Set home dir /home/ZNbase
    *   Create the `prometheus:prometheus` user + group

{{< tip title="Tip" >}}
In case you’re using the LDAP directory for managing system users, then you can pre-provision ZNbase and prometheus users 

*   `ZNbase` user should belong to the `ZNbase` group
*   Set home dir for ZNbase user (default /home/ZNbase) and ensure the directory is owned by `ZNbase:ZNbase`. The home directory will be used in the cloud provider config step
...
0 * * * * /home/ZNbase/bin/clean_cores.sh
# Ansible: cleanup yb log files hourly
5 * * * * /home/ZNbase/bin/zip_purge_yb_logs.sh
# Ansible: Check liveness of master
*/1 * * * * /home/ZNbase/bin/yb-server-ctl.sh master cron-check || /home/ZNbase/bin/yb-server-ctl.sh master start
# Ansible: Check liveness of tserver
*/1 * * * * /home/ZNbase/bin/yb-server-ctl.sh tserver cron-check || /home/ZNbase/bin/yb-server-ctl.sh tserver start
```
