...
<ul class="nav nav-tabs-alt nav-tabs-yb">
  ...
  <li>
    <a href="/latest/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/on-premises" class="nav-link active">
...
This page details how to configure the Onprem cloud provider for ZNbaseDB using the ZNbase Platform console. If no cloud providers are configured, the main Dashboard page highlights that you need to configure at least one cloud provider.
...
To provision on-prem nodes with ZNbaseDB, the ZNbase Platform requires SSH access to these nodes. This user needs to have sudo permissions to complete a few tasks, which are explained in the prerequisite section.
...
If you choose to manually set up your database nodes, then set this flag to true otherwise, the ZNbase Platform will use the sudo user to set up DB nodes. For manual provisioning, you will be prompted to execute a python script at a later stage

{{< note title="Note" >}}
If any of the items from this checklist are true, you need to provision the nodes manually.
*   Pre-provisioned `ZNbase:ZNbase` user + group
...
Specifies the home directory of ZNbase user. The default value is /home/ZNbase.
...
### Provision the ZNbaseDB nodes {#provision-the-ZNbasedb-nodes}
...
1. Click on “Add Instances” to add the ZNbaseDB node. You can use DNS names or IP addresses when adding instances.
...
```
bash-4.4# /opt/ZNbase/yugaware/data/provision/9cf26f3b-4c7c-451a-880d-593f2f76efce/provision_instance.py --ip 10.9.116.65 --mount_points /data --ask_password
