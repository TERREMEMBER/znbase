...
description: How to migrate an application written for PostgreSQL to ZNbaseDB.
menu:
  latest:
...
This section outlines the recommended changes for porting an existing PostgreSQL application to ZNbaseDB.

## Retry transactions on conflicts

Currently (as of ZNbaseDB v2.2), only a subset of transactions that get aborted due to an internal conflict are retried transparently. ZNbaseDB uses the error code 40001 (serialization_failure) for retryable transaction conflict errors. We recommend retrying the transactions from the application upon encountering these errors.

{{< note title="Note" >}}
This is the state as of ZNbaseDB v2.2, reducing transaction conflicts by transparently handling retries of most transactions transparently is work in progress.
{{< /note >}}
