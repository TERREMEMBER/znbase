...
In this tutorial, you are first going to setup [JanusGraph](https://janusgraph.org/) to work with ZNbaseDB as the underlying database. Then, using the Gremlin console, you are going to load some data and run some graph commands.

## 1. Start local cluster

Start a cluster on your [local machine](../../../quick-start/install/). Check that you are able to connect to ZNbaseDB using `ycqlsh` by doing the following.
...
- Now use the YCQL config to initialize JanusGraph to talk to ZNbase.
...
- Open the ZNbaseDB UI to verify that the `janusgraph` keyspace and the necessary tables were created by opening the following URL in a web browser: `http://localhost:7000/` (replace `localhost` with the IP address of any master node in a remote deployment). You should see the following.
