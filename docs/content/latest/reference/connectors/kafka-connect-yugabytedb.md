...
headerTitle: Kafka Connect ZNbaseDB
linkTitle: Kafka Connect ZNbaseDB
description: Use Kafka Connect ZNbaseDB to stream ZNbaseDB table updates to Kafka topics.
section: REFERENCE
menu:
  latest:
...
2. Install ZNbaseDB and create the database table.
    ...
    Create a database and table by running the following command. You can find `ycqlsh` in the `bin`  subdirectory located inside the ZNbaseDB installation folder.
...
3. Set up and run the Kafka Connect Sink.
    ...
    ```sh
    $ ~/yb-kafka/kafka_2.12-2.5.0/bin/connect-standalone.sh ~/yb-kafka/yb-kafka-connector/resourcesexamples/kafka.connect.properties ~/yb-kafka/yb-kafka-connector/resources/examplesZNbase.sink.properties
    ```
