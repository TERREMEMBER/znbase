...
aliases:
  - /latest/deploy/ZNbase-cloud/create-databases/
menu:
  latest:
...
You can use the YSQL shell (`ysqlsh`) and the [YSQL API](../../api/ysql/) to create and manage ZNbaseDB databases and tables in your ZNbase Cloud clusters.
...
You can use the YCQL shell (`ycqlsh`) and the [YCQL API](../../api/ycql/) to create and manage ZNbaseDB keyspaces and tables in your ZNbase Cloud clusters.
...
[Sample datasets](../../sample-data/) are available to help you learn and explore ZNbaseDB. The SQL script files (`.sql`) required to create
sample databases are included in the `share` directory of the ZNbaseDB home directory.
