...
description: Connect to clusters in ZNbase Cloud using locally installed ZNbaseDB clients.
...
You can connect to ZNbaseDB clusters in ZNbase Cloud with ZNbaseDB clients and third party clients.

You can use the following ZNbaseDB clients (locally installed) to connect to your remote clusters in ZNbase Cloud:
...
To use the ZNbaseDB CLIs to connect to your remote ZNbase Cloud clusters, you must have a local installation of
ZNbaseDB. If you do not have a local installation, see [Install ZNbaseDB](../../../quick-start/install/).
...
1. Log into ZNbase Cloud and click **Cluster** in the navigation bar. The list of available clusters appears.
2. Click **Go to cluster** for your cluster. The ZNbase Cloud Console appears.
3. Click **Connect** to get the user credentials you need to access the remote cluster. The Connect dialog appears with user credentials
   ...
   the host (`-h`), port (`-p`), username (`-U`), and database (`-d`). To specify the password The generated command connects to the default database (`ZNbase`).

   Here's an example of the generated command:

    ```sh
    PGPASSWORD=sx73qlpc ./bin/ysqlsh -h 35.236.85.97 -p 12201 -U admin -d ZNbase
    ```

4. Change directories to the ZNbase home directory.
...
Because ZNbaseDB is PostgreSQL-compatible, you can use third party PostgreSQL clients to connect to your ZNbaseDB clusters in ZNbase Cloud.
To create connections to your cluster in ZNbase Cloud, follow the client's configuration steps for PostgreSQL, but use the values for host, port, username,
