...
description: Use the CREATE ROLE statement to add a new role to a ZNbaseDB database cluster.
block_indexing: true
menu:
  v2.1:
...
Use the `CREATE ROLE` statement to add a new role to a ZNbaseDB database cluster. A role is an entity that can own database objects and have database privileges.
