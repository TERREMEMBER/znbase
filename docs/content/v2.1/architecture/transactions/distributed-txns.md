...
Distributed ACID transactions are transactions that modify multiple rows in more than one shard. ZNbaseDB supports distributed transactions, enabling features such as strongly consistent secondary indexes and multi-table/row ACID operations in both the YCQL context as well as in the YSQL context. This section provides some common concepts and notions used in ZNbase's approach to implementing distributed transactions.  Once you are familiar with these concepts, see [Transactional IO Path](../transactional-io-path/) for a walk-through of a distributed transaction's life cycle.

## Provisional records

Just as ZNbaseDB stores values written by single-shard ACID transactions into
...
and thus breaking atomicity.  ZNbaseDB therefore writes *provisional records* to all tablets
...
none are visible at all. ZNbaseDB already provides atomicity of single-shard updates by
...
To continue exploring the architecture of ZNbaseDB's distributed transaction implementation, take a look at the [Core functions / IO path with distributed transactions](../transactional-io-path/) section next.
