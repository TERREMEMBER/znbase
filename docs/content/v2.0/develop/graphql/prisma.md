...
If you are new to ZNbaseDB, you can be up and running with ZNbaseDB in under five minutes by following the steps in [Quick start](/latest/quick-start/). After installing ZNbaseDB, make sure to follow the step mentioned above.
...
In order to quickly explore using Prisma with ZNbaseDB, we will use the default database and user in the PostgreSQL-compatible ZNbaseDB.
