...
The yb-master-n1 home page shows that we have a cluster (or universe) with `Replication Factor` of 1 and `Num Nodes (TServers)` as 1. The `Num User Tables` is `0` since there are no user tables created yet. ZNbaseDB version number is also shown for your reference.
