...
The `yb-master-0` home page shows that we have a cluster with **Replication Factor** of 1 and **Num Nodes (TServers)** as `1`. The **Num User Tables** is `0` because there are no user tables created yet. The ZNbaseDB version is also displayed for your reference.
