...
The `DELETE` statement removes rows from a specified table that meet a given condition. Currently, ZNbaseDB can deletes one row at a time. Deleting multiple rows is not yet supported.
