...
- Currently ZNbaseDB will always use the snapshot isolation level internally. For more information, see [Transaction isolation levels](../../../../architecture/transactions/isolation-levels/).
...
```postgresql
ZNbase=# BEGIN TRANSACTION; SET TRANSACTION ISOLATION LEVEL REPEATABLE READ; 
```

```postgresql
ZNbase=# INSERT INTO sample(k1, k2, v1, v2) VALUES (1, 2.0, 3, 'a'), (1, 3.0, 4, 'b');
```
...
```postgresql
ZNbase=# INSERT INTO sample(k1, k2, v1, v2) VALUES (2, 2.0, 3, 'a'), (2, 3.0, 4, 'b');
```
...
```postgresql
ZNbase=# SELECT * FROM sample; -- run in first shell
```
...
```postgresql
ZNbase=# SELECT * FROM sample; -- run in second shell
```
...
```postgresql
ZNbase=# COMMIT TRANSACTION; -- run in first shell.
```

Abort the current transaction (from the first shell).

```postgresql
ZNbase=# ABORT TRANSACTION; -- run second shell.
```
