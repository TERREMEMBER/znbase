...
For correctness, in addition to disabling the WAL mechanism in RocksDB, ZNbaseDB tracks the Raft
...
overhead. ZNbaseDB does not use RocksDB’s sequence ids, and instead uses hybrid-timestamps that are
