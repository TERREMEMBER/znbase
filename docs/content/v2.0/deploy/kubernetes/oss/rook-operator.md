...
A ZNbaseDB cluster installed in a Kubernetes environment is required.

- To create a local cluster in Kubernetes for development and learning, see [Quick start](/latest/quick-start/).
- To deploy a production cluster, see the ZNbaseDB documentation on [deploying in  Kubernetes](../../../kubernetes/).
...
```console
kubectl exec -it yb-tserver-rook-ZNbasedb-0 -- /home/ZNbase/bin/ysqlsh -h yb-tserver-rook-ZNbasedb-0  --echo-queries
```
