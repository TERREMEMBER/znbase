This page documents bulk import for ZNbaseDB’s [Cassandra compatible YCQL API](../../../api/ycql).

We will first export data from existing Apache Cassandra and MySQL tables. Thereafter, we will import the data using the various bulk load options supported by ZNbaseDB. We will use a generic IoT timeseries data use case as a running example to illustrate the import process.
...
[`cassandra-loader`](https://github.com/brianmhess/cassandra-loader) is a general purpose bulk loader for CQL that supports various types of delimited files (particularly csv files). For more details, review the README of the [ZNbaseDB cassandra-loader fork](https://github.com/ZNbase/cassandra-loader/). Note that cassandra-loader requires quotes for collection types (e.g. “[1,2,3]” rather than [1,2,3] for lists).
...
For large datasets that are in the order of terabytes, ZNbaseDB's bulk-importer is the tool to be used. Currently, it is supported only for AWS based deployments. Further documentation on this topic will be added soon. Meanwhile, reach out to [support@ZNbase.com](mailto:support@ZNbase.com) for more details.
