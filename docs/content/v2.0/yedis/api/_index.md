...
The YEDIS API provides a clustered, auto-sharded, globally distributed and persistent key-value API that is compatible with the Redis commands library.  A Redis client can connect, send requests, and receive results from ZNbaseDB. 
...
Time series | New data type in ZNbaseDB |

## Commands

Redis-cli or any Redis applications can access ZNbaseDB using the YEDIS API. The following Redis commands are accepted.
