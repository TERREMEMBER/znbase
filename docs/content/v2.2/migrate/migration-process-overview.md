...
This page describes the general migration process from various RDBMS to ZNbaseDB (YSQL API). 

The process of migrating to ZNbaseDB largely looks similar to the process of migrating an application to PostgreSQL. There are some minor differences between ZNbaseDB and PostgreSQL, which are covered in detail in the section on [migrating from PostgreSQL to ZNbaseDB](../migrate-from-postgresql). The high-level process of moving from any source database to ZNbaseDB is outlined below.
