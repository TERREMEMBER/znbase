...
description: Use ZNbase Platform to enable encryption in transit (TLS) on a ZNbaseDB universe and connect to clients.
menu:
  v2.2:
    parent: security
...
ZNbase Platform provides the ability to protect data in transit with:
...
ZNbase Platform provides the option to create a new self-signed certificate, use an existing self-signed certificate, or upload a third-party certificate from external providers, such as Venafi or DigiCert (only available for manual installations).
...
To view the certificate, select the drop-down list in the top-right corner of the ZNbase Platform console and click **Certificates**.

## Enable TLS using a self-signed, user-provided certificate

1. In the top-right corner of the ZNbase Platform console, click the drop-down list (with the profile icon) and select **Certificates**.
...
This will download two files: `ZNbasedb.crt` and `ZNbasedb.key`.

2. For testing with a `ysqlsh` client, copy these files to `<home-dir>/.ZNbasedb` directory and change the permissions to `0600`.

```sh
$ mkdir ~/.ZNbasedb; cd ~/.ZNbasedb
$ cp <DownloadDir>/ZNbasedb.crt .
$ cp <DownloadDir>/ZNbasedb.key .
$ chmod 600 ZNbasedb.*
```
...
```sh
$ cd <ZNbase software install directory>
...
To use TLS from a different client, see the client-specific documentation. For example, if you’re using a Postgres JDBC driver to connect to ZNbaseDB, see [Using SSL – Configuring the Client](https://jdbc.postgresql.org/documentation/head/ssl-client.html) for more details.
...
```sh
$ cp <DownloadDir>/root.crt ~/.ZNbasedb/root.crt
$ export SSL_CERTFILE=~/.ZNbasedb/root.crt
...
To use TLS from a different client, see the client-specific documentation. For example, if you’re using a Cassandra driver to connect to ZNbaseDB, see [Security – SSL](https://docs.datastax.com/en/developer/python-driver/3.19/security/#ssl) for more details.
