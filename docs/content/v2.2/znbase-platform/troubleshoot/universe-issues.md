...
In the [Admin Console](../../deploy/install-ZNbase-platform/), select the [Universe](../../../architecture/concepts/universe/) page, then go to the **Metrics** tab. The page shows a number of interactive metrics graphs that capture the state of the ZNbase Universe over time.

![ZNbase Metrics Page](/images/troubleshooting/check-metrics.png)
...
In the [Admin Console](../../deploy/install-ZNbase-platform/), click on the [Universe](../../../architecture/concepts/universe/) page, then go to the **Nodes** tab.
The page will show the status of the Master and TServer on each ZNbaseDB node.

![ZNbase Nodes Page](/images/troubleshooting/check-node-status.png)
...
To check host resources on your ZNbaseDB nodes, run the following script, replacing the IP addresses with the IP addresses of your ZNbaseDB nodes.
