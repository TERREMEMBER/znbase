---
title: Configure the Microsoft Azure provider
headerTitle: Configure the Microsoft Azure provider
linkTitle: Configure the cloud provider
description: Configure the Microsoft Azure cloud provider.
menu:
  v2.2:
    identifier: set-up-cloud-provider-3-azure
    parent: configure-ZNbase-platform
    weight: 20
isTocNested: true
showAsideToc: false
---

<ul class="nav nav-tabs-alt nav-tabs-yb">

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/aws" class="nav-link">
      <i class="fab fa-aws"></i>
      AWS
    </a>
  </li>

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/gcp" class="nav-link">
      <i class="fab fa-google" aria-hidden="true"></i>
      GCP
    </a>
  </li>

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/azure" class="nav-link active">
      <i class="icon-azure" aria-hidden="true"></i>
      Azure
    </a>
  </li>

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/kubernetes" class="nav-link">
      <i class="fas fa-cubes" aria-hidden="true"></i>
      Kubernetes
    </a>
  </li>

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/vmware-tanzu" class="nav-link">
      <i class="fas fa-cubes" aria-hidden="true"></i>
      VMware Tanzu
    </a>
  </li>

  <li>
    <a href="/v2.2/ZNbase-platform/configure-ZNbase-platform/set-up-cloud-provider/on-premises" class="nav-link">
      <i class="fas fa-building"></i>
      On-premises
    </a>
  </li>

</ul>

Support for Microsoft Azure in ZNbase Platform is currently in the works.
Until support is added, you can configure Microsoft Azure as an
[on-premises data center](../install-ZNbase-platform).
