...
description: Learn how ACID transactions work in YCQL on ZNbaseDB.
block_indexing: true
menu:
  v2.2:
...
<ul class="nav nav-tabs-alt nav-tabs-yb">
  ...
  <li >
    <a href="/v2.2/develop/learn/acid-transactions-ysql" class="nav-link">
  ...
  <li >
    <a href="/v2.2/develop/learn/acid-transactions-ycql" class="nav-link active">
...
- **Isolation** This property determines how/when changes made by one transaction become visible to the other. For example, a *serializable* isolation level guarantees that two concurrent transactions appear as if one executed after the other (i.e. as if they occur in a completely isolated fashion). ZNbaseDB supports both *Snapshot Isolation*, and *Serializable* isolation levels. Read more about the different [levels of isolation](../../../architecture/transactions/isolation-levels/).
...
Browse the [Java source code for the batch application](https://github.com/ZNbase/ZNbase-db/blob/master/java/yb-loadtester/src/main/java/com/ZNbase/sample/apps/CassandraTransactionalKeyValue.java) to see how everything fits together.
...
By default, the original Cassandra Java driver and the ZNbaseDB Cassandra Java driver use `com.datastax.driver.core.policies.DefaultRetryPolicy` which can retry requests upon timeout on client side.

Automatic retries can break linearizability of operations from the client point of view. Therefore you have added `com.ZNbase.driver.core.policies.NoRetryOnClientTimeoutPolicy` which inherits behavior from DefaultRetryPolicy with one exception - it results in an error in case the operation times out (with the `OperationTimedOutException`).
...
To avoid such linearizability issues, use `com.ZNbase.driver.core.policies.NoRetryOnClientTimeoutPolicy` and handle client timeouts in the application layer.
