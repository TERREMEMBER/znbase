...
```plpgsql
ZNbase=# ALTER ROLE John WITH PASSWORD 'new_password';
```

- Rename a role.

```plpgsql
ZNbase=# ALTER ROLE John RENAME TO Jane;
```

- Change default_transaction_isolation session parameter for a role.

```plpgsql
ZNbase=# ALTER ROLE Jane SET default_transaction_isolation='serializable';
```
