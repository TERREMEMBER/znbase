...
char* DocHybridTime::EncodedInDocDbFormat(char* dest) const {
  ...
  // the "ZNbase epoch" as a signed operation, so that we can still represent hybrid times earlier
  // than the ZNbase epoch.
  char* out = dest;
  ...
  out = FastEncodeDescendingSignedVarInt(
      static_cast<int64_t>(hybrid_time_.GetPhysicalValueMicros() - kZNbaseMicrosecondEpoch),
      out);
...
Status DocHybridTime::DecodeFrom(Slice *slice) {
  const size_t previous_size = slice->size();
  {
    ...
    int64_t decoded_micros =
        kZNbaseMicrosecondEpoch + VERIFY_RESULT(FastDecodeDescendingSignedVarInt(slice));
