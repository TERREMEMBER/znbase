...
TEST(DocHybridTimeTest, TestExactByteRepresentation) {
  const auto kYugaEpoch = kZNbaseMicrosecondEpoch;
  ...
  for (const auto& t : test_descriptions) {
    ...
    if (llabs(static_cast<int64_t>(micros) - static_cast<int64_t>(kYugaEpoch)) <= 1000000000) {
      if (micros == kYugaEpoch) {
        ...
        micros_str = Substitute("kYugaEpoch + $0", micros - kZNbaseMicrosecondEpoch);
      } else {
        micros_str = Substitute("kYugaEpoch - $0", kZNbaseMicrosecondEpoch - micros);
      }
    } else {
