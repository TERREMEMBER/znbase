...
GetCurrentFDWTuplestore(AfterTriggerShared evtshared)
{
	...
	const bool is_deferred = IsZNbaseEnabled() && afterTriggerCheckState(evtshared);
	...
	if (ret == NULL)
	{
		 ...
		 * In ZNbase mode deferred trigger will access tuplestore
		...
		for_each_event(event, chunk)
		{
				...
				evtshared->ats_firing_id == firing_id)
			{
				...
				if (rel == NULL || RelationGetRelid(rel) != evtshared->ats_relid)
				{
					...
					/*
					 * Need to create a tuple slot for both ZNbase tables and
					 * foreign tables
					 */
					  ...
					  TransitionCaptureState *transition_capture)
{
	...
	/*
	 * In ZNbase mode we (re)use the FDW trigger flags (since we also use the
	 * FDW tuplestore).
	 */
	...
	for (i = 0; i < trigdesc->numtriggers; i++)
	{
		...
		/*
		 * In ZNbase mode we also use the tuplestore to store/pass tuples
		...
		if ((IsYBBackedRelation(rel) || relkind == RELKIND_FOREIGN_TABLE) && row_trigger)
		{
			if (IsZNbaseEnabled() && afterTriggerCheckState(&new_shared))
			{
