...
YBReadSequenceTuple(Relation seqrel)
{
  ...
  if (IsZNbaseEnabled())
  {
    ...
    seqdataform.log_cnt = 0; /* not used by ZNbase, defaults to 0 */
  }
  else
  {
    ereport(ERROR,
...
check_bounds:
	...
	/*
	 * ZNbase doesn't use the WAL, and we don't need to free the buffer because we didn't allocate
...
do_setval(Oid relid, int64 next, bool iscalled)
{
	...
	/*
	 * Only read the sequence from a disk page if we are in Postgres mode, since ZNbase stores it
	...
	/*
	 * Update the sequence in the ZNbase backend. ZNbase doesn't use the WAL, and we
