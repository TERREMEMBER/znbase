			 ...
			 IndexInfo *indexInfo)
{
	...
	/*
	 * For ZNbase-based index, call the variant of aminsert that takes the full tuple instead of
	 * the tuple id.
	 */
	if (IsZNbaseEnabled() && IsYBRelation(indexRelation))
	{
 ...
 *      This is used only for indexes backed by ZNbaseDB. For Postgres, when a tuple is updated,
 ...
 *      ZNbase-based tables, the ybctid is the primary key of the tuple and will remain valid
...
{
	/*
	 * For ZNbase secondary indexes, there are two scenarios.
	 * - If ZNbase returns an index-tuple, the returned ybctid value should be used to query data.
	 * - If ZNbase returns a heap_tuple, all requested data was already selected in the tuple.
	 */
	if (IsZNbaseEnabled())
	{
		if (scan->xs_hitup != 0)
