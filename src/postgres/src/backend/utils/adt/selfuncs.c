...
patternsel(PG_FUNCTION_ARGS, Pattern_Type ptype, bool negate)
{
	...
	{
		case TEXTOID:
			opfamily = IsZNbaseEnabled() ? TEXT_LSM_FAM_OID : TEXT_BTREE_FAM_OID;
			break;
		case BPCHAROID:
			opfamily = IsZNbaseEnabled() ? BPCHAR_LSM_FAM_OID : BPCHAR_BTREE_FAM_OID;
			break;
		case NAMEOID:
			opfamily = IsZNbaseEnabled() ? NAME_LSM_FAM_OID : NAME_BTREE_FAM_OID;
			break;
		case BYTEAOID:
			opfamily = IsZNbaseEnabled() ? BYTEA_LSM_FAM_OID : BYTEA_BTREE_FAM_OID;
			break;
		default:
			ReleaseVariableStats(vardata);
			return result;
