...
readonly DEFAULT_YSQL_USER="ZNbase"
readonly DEFAULT_YSQL_PASSWORD="ZNbase"
readonly DEFAULT_YSQL_DB="ZNbase"
...
readonly ZNbaseD_BINARY="bin/ZNbased"
readonly PARENT_TEST_DIR="/tmp/test-ZNbased/ZNbased-test-$( date +%Y-%m-%dT%H_%M_%S )-$RANDOM"
readonly EXTRACTED_YB_TAR="${PARENT_TEST_DIR}/ZNbase-db"
readonly DOWNLOADED_TAR="${PARENT_TEST_DIR}/tar"
readonly TEST_BASE_DIR="${PARENT_TEST_DIR}/base_dir"

TEST_IMAGE="ZNbased-test:latest"
...
Options:
  ...
  -p, --package
    ZNbaseDB package (release tar).
  -i, --image
    ZNbaseDB Docker image.
  ...
  Advanced:
  --master_flags
    Set YB-Master gflags - comma-seperated string, passed directly to ZNbased.
  --tserver_flags
    Set YB-Tserver gflags -comma-seperated string, passed directly to ZNbased.

  Note: Custom combination of test cases (-t) can be used to test single combination externally.

  Examples:
    1. With package and custom ZNbased.
      ZNbased-test.sh -p /path/package.tar.gz -y /path/ZNbased

    2. With package and advanced test suite.
      ZNbased-test.sh -p /path/package.tar.gz -T advanced

    3. With docker image and package.
      ZNbased-test.sh -p /path/package.tar.gz -i image:tag

    4. With docker image and remote package url
      ZNbased-test.sh -p https://example.com/package.tar.gz -i image:tag

    5. With custom test case combination
      ZNbased-test.sh -p /path/package.tar.gz -t "custom_ysql_port,custom_ycql_port"

    6. Advanced flags
      ZNbased-test.sh -p package.tar.gz --master_flags="flag_name=flag_value,flag_name=flag_value"
EOT
}
...
ZNbased_start() {
  ...
  # Set advanced flags directly to ZNbased.
  ...
  if [[ "${DOCKER_ENV}" == "true" ]]; then
    ...
    ${DOCKER_ENV_VARS[@]-} \
    ${TEST_IMAGE} ${ZNbaseD_BINARY} start "${start_args_array[@]}")

    log "Container ID: ${CONTAINER_ID}"
    CONTAINER_IDS+=("$CONTAINER_ID")
  else
    "${python_interpreter}" "${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}" start "${start_args_array[@]}"
  fi
}
...
ZNbased_stop() {
  ...
  if [[ "${DOCKER_ENV}" == "true" ]]; then
    ...
    if [[ -z "${stop_args_array[*]-}" ]]; then
      "${python_interpreter}" "${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}" stop
    else
      "${python_interpreter}" "${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}" stop "${stop_args_array[@]}"
    fi
  fi
}
...
setup_test_env() {
  ...
  # Replace ZNbased in package
  if [[ -n "$ZNbased" ]]; then
    if ! [[ "${ZNbased}" -ef "${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}" ]]; then
      cp -f "${ZNbased}" "${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}"
      log "ZNbased copied Successfully."
    fi
  fi
...
FROM ${docker_image}
COPY ZNbased /home/ZNbase/${ZNbaseD_BINARY}
EOL

      cp -f "${ZNbased}" "${PARENT_TEST_DIR}/docker/ZNbased"
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 ]]; then
    ...
    if [[ "${DOCKER_ENV}" == "false" ]]; then
      # Kill ZNbased forcefully
      pkill -f "${python_interpreter} ${EXTRACTED_YB_TAR}/${ZNbaseD_BINARY}" -SIGKILL
    fi
  fi
...
declare -a ZNbased_start_args
declare -a ZNbased_restart_args
declare -a ZNbased_stop_args
declare -a ysql_args
declare -a ycql_args
...
execute_test_case() {
  ...
  set_env_vars
  ZNbased_start "${ZNbased_start_args[@]-}"
  verify_ysqlsh "${ysql_args[@]-}"
  verify_ycqlsh "${ycql_args[@]-}"
  ZNbased_stop "${ZNbased_stop_args[@]-}"
  unset_env_vars

  # Restart the ZNbased to verify the configuration persist or not.
  # In Docker, it won't verify the same.
  if [[ "${DOCKER_ENV}" == "false" ]]; then
    log_heading "Restarting with previous configuration"
    ZNbased_start "${ZNbased_restart_args[@]-}"
    verify_ysqlsh "${ysql_args[@]-}"
    verify_ycqlsh "${ycql_args[@]-}"
    ZNbased_stop "${ZNbased_stop_args[@]-}"
...
base_dir() {
  ZNbased_start_args+=(--base_dir "${TEST_BASE_DIR}")
  ZNbased_restart_args+=(--base_dir "${TEST_BASE_DIR}")
  ZNbased_stop_args+=(--base_dir "${TEST_BASE_DIR}")
}

custom_ysql_port() {
  ZNbased_start_args+=(--ysql_port "$TEST_YSQL_PORT")
  ysql_args+=(--port "$TEST_YSQL_PORT")
}

custom_ycql_port() {
  ZNbased_start_args+=(--ycql_port "$TEST_YCQL_PORT")
  ycql_args+=(--port "$TEST_YCQL_PORT")
}

default_ysql_authentication() {
  ZNbased_start_args+=(--ysql_enable_auth true)
  ysql_args+=(--auth true)
  ENV_VARS+=(PGPASSWORD="$DEFAULT_YSQL_PASSWORD")
}

default_ycql_authentication() {
  ZNbased_start_args+=(--use_cassandra_authentication true)
  ycql_args+=(--auth true)
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -p|--package)
      ...
      package=$1
      log "ZNbaseDB Package: ${package}"
    ;;
    -i|--image)
      shift
      is_empty "${1-}" "Docker image"
      docker_image=$1
      log "ZNbaseDB Docker Image: ${docker_image}"
    ;;
    -y|--ZNbased)
      shift
      is_empty "${1-}" "ZNbased"
